...
class WalkController {
    ...
    public static function add(Request $request, Response $response) {
        ...
        $schema = json_decode(file_get_contents(WWW_ROOT . "api" . DIRECTORY_SEPARATOR . "schema.json"));
        $json = json_decode($request->getBody()->getContents());
        $result = Jsv4::validate($json, $schema);

        if(!$result->valid) {
            return self::encode(array('errors'=>$result->errors), $response);
        }

        // Parse walk object
        $walk_id = WalkDAO::addWalk($data['translations'], $data['theme_id'], $data['walk_duration'], $data['walk_distance'], $data['walk_thumbnail']);
    ...
    public static function addPoiToWalk(Request $request, Response $response) {
        $data = $request->getParsedBody();
    ...
    public static function addWaypointToWalk(Request $request, Response $response) {
        $data = $request->getParsedBody();
        $location_id = LocationDAO::addLocation($data['location']['translations'], $data['location']['location_lat'], $data['location']['location_lon'], (int) $data['location']['location_house_number'], $data['location']['location_postal_code']);
    ...
    public static function getPoisByWalkID(Request $request, Response $response) {
        return self::encode(WalkDAO::getAllPois($request->getAttribute('id'), LanguageDAO::getLanguageIDByCode($request->getQueryParam('lang'))), $response);
    }

    public static function getWaypointsByWalkID(Request $request, Response $response) {
        return self::encode(WalkDAO::getAllWaypoints($request->getAttribute('id'), LanguageDAO::getLanguageIDByCode($request->getQueryParam('lang'))), $response);
    }
    ...
    public static function deleteByID(Request $request, Response $response) {
        return self::encode(WalkDAO::deleteWalkByID($request->getAttribute('id')), $response);
    }
